...
# 加载剪枝并训练好的模型
model_path = save_path + 'model_quantized_ih0_hh0_ih1_hh1.pth'
...
with torch.no_grad():
    ...
    model.rnn.weight_hh_l1 = Q(model.rnn.weight_hh_l1)
    model.rnn.bias_hh_l1 = Q(model.rnn.bias_hh_l1)

    model.fc.weight = Q(model.fc.weight)
...
def get_trainable_params(model):
    for name, param in model.named_parameters():
        ...
        if name not in ['rnn.weight_ih_l0','rnn.bias_ih_l0','rnn.weight_hh_l0','rnn.bias_hh_l0',\
                        'rnn.weight_ih_l1','rnn.bias_ih_l1','rnn.weight_hh_l1','rnn.bias_hh_l1',\
                        'fc.weight']:
            yield param
...
for epoch in range(epochs):
    ...
    print(f'Epoch {epoch+1}: Train Loss: {loss.item()}, Test Loss: {epoch_test_loss}')
'''
print('model_quantized_ih0_hh0_ih1_hh1_fc Done!')
# 保存量化模型

test_loss = test_model(model, test_dl)
print(f"Test Loss After quantization: {test_loss}")
torch.save(model, save_path + 'model_quantized_ih0_hh0_ih1_hh1_fc.pth')